...
<body>
    <div class="container">
        ...
        <!-- 송금 폼 -->
        <form action="validateTransferAction.tr" method="get" autocomplete="off">
        	<!-- 보내는 계좌 -->
        	<input type="hidden" name="senderAccount" value="${accountNum}">
            ...
            <div class="mb-3">
                ...
                <input type="text" id="receiverAccount" name="receiverAccount" class="form-control" maxlength="13" required>
            </div>
                ...
                <div class="d-flex flex-wrap gap-2">
                    <c:forEach var="recentAccount" items="${recentAccounts}">
                        <button type="button" class="btn btn-outline-secondary" onclick="selectAccount('${recentAccount.accountNumber}')">
                            ${recentAccount.accountName} (${recentAccount.account_num})
                        </button>
                    </c:forEach>
                </div>
            </div>
                ...
                <div class="d-flex flex-wrap gap-2">
                    <c:forEach var="myAccount" items="${myAccounts}">
                        <button type="button" class="btn btn-outline-secondary" onclick="selectAccount('${myAccount.accountNumber}')">
                            ${myAccount.accountName} (${myAccount.account_num})
                        </button>
                    </c:forEach>
                </div>
            </div>
                ...
                <div class="d-flex flex-wrap gap-2">
                    <c:forEach var="favoriteAccount" items="${favoriteAccounts}">
                        <button type="button" class="btn btn-outline-secondary" onclick="selectAccount('${favoriteAccount.accountNumber}')">
                            ${favoriteAccount.accountName} (${favoriteAccount.account_num})
                        </button>
                    </c:forEach>
                </div>
            </div>
