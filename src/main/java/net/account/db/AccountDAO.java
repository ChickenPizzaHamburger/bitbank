...
public class AccountDAO {
    ...
    public boolean createAccount(AccountBean account, HttpServletRequest request) {
        ...
        try {
            ...
            System.err.println("통장 개설 실패 : " + e);
        }

        return result;
    }
 // 계좌 번호로 계좌 정보를 조회하는 메소드
        ...
        try (Connection con = ds.getConnection();
            ...
            try (ResultSet rs = pstmt.executeQuery()) {
                if (rs.next()) {
                    ...
                    account.setAccount_type(AccountBean.Type.valueOf(rs.getString("account_type")));
                    account.setAccount_amount(rs.getLong("account_amount"));
                    account.setAccount_date(rs.getDate("account_date").toLocalDate());
    ...
    // 사용자 계좌 정보를 조회하는 메소드
    public List<AccountBean> getUserAccounts(String userId) {
